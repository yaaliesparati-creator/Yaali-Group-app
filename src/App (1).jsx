...
const seedServicios = {
  inmobiliaria: [
    { id: 1, nombre: "Asesoría de compra", precio: 5000, descripcion: "Acompañamiento en proceso de compra INFONAVIT/FOVISSSTE/Bancario", activo: true },
  ...
  return (
    <div>
      <label style={{ fontSize:12, fontWeight:700, color:C.gray700, textTransform:"uppercase", letterSpacing:.5, display:"block", marginBottom:8 }}>
        Fotos de la propiedad <span style={{color:C.gray400,fontWeight:400,textTransform:"none"}}>({fotos.length}/{MAX})</span>
      </label>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:10 }}>
        {fotos.map((src,i) => (
          <div key={i} style={{ position:"relative", aspectRatio:"1", borderRadius:8, overflow:"hidden", background:C.gray50 }}>
            ...
            <button onClick={()=>remove(i)} style={{ position:"absolute", top:4, right:4, background:"rgba(0,0,0,.6)", border:"none", color:"#fff", borderRadius:"50%", width:22, height:22, fontSize:13, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", lineHeight:1 }}>×</button>
      ...
      {fotos.length === 0 && (
        <div style={{ textAlign:"center", padding:"16px 0", color:C.gray400, fontSize:12 }}>
          Toca 📷 para agregar hasta {MAX} fotos de la propiedad
        </div>
      )}
...
function ClientesModule({ biz, clientes, onChange, toast }) {
  ...
  return (
    <div>
      ...
      {filtered.length===0 && <div style={{textAlign:"center",color:C.gray400,padding:40}}>No hay clientes aún.</div>}
...
function InventarioModule({ inventario, onChange, toast }) {
  ...
  const [search, setSearch] = useState("");
  const [viewer, setViewer] = useState(null); // {fotos, idx}
  ...
  const openEdit = (p) => {
    setForm({...p, precio:String(p.precio), recamaras:String(p.recamaras), banos:String(p.banos), m2:String(p.m2)});
          ...
          <Card key={p.id} style={{padding:0,overflow:"hidden"}}>
            ...
            {p.fotos?.length>0 ? (
              <div style={{position:"relative",height:180,background:C.gray100,cursor:"pointer"}} onClick={()=>setViewer({fotos:p.fotos,idx:0})}>
                ...
                <div style={{position:"absolute",bottom:8,right:8,background:"rgba(0,0,0,.6)",color:"#fff",borderRadius:12,padding:"3px 10px",fontSize:11,fontWeight:700}}>📷 {p.fotos.length} fotos</div>
                <div style={{position:"absolute",top:8,left:8}}><Badge text={p.status} color={statusColor[p.status]||C.teal}/></div>
              </div>
            ) : (
              <div style={{height:80,background:`linear-gradient(135deg,${C.teal}22,${C.teal}11)`,display:"flex",alignItems:"center",justifyContent:"center",gap:8,color:C.gray400,fontSize:13}}>
                <span style={{fontSize:24}}>🏠</span> Sin fotos
              </div>
            )}
      ...
      {stats.map(s=>(
        <Card key={s.biz} style={{borderLeft:`4px solid ${s.color}`}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
            {[{label:"Clientes",val:s.clientes},{label:"Activos",val:s.activos},{label:"Servicios",val:s.servicios}].map(x=>(
...
export default function App() {
  ...
  const updateClientes  = async (biz,list) => { const n={...data,clientes:{...data.clientes,[biz]:list}}; setData(n); await save(`yaali_clientes_${biz}`,list); };
  const updateServicios = async (biz,list) => { const n={...data,servicios:{...data.servicios,[biz]:list}}; setData(n); await save(`yaali_servicios_${biz}`,list); };
  const updateInventario= async (list) => { setData({...data,inventario:list}); await save("yaali_inventario",list); };
  ...
  return (
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:C.offWht,minHeight:"100vh",maxWidth:480,margin:"0 auto",paddingBottom:80}}>
      ...
      <div style={{padding:"16px 16px 0"}}>
        ...
        {nav!=="dashboard"&&tab==="clientes"&&<ClientesModule biz={nav} clientes={data.clientes[nav]||[]} onChange={list=>updateClientes(nav,list)} toast={toast}/>}
